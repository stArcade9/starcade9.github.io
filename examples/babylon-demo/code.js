...
export function init() {
  ...
  nova64.light.setFog(0x0a0a1a, 15, 60);

  // Ground platform
  platform = nova64.scene.createPlane(20, 20, 0x2a2a4a, [0, -0.01, 0]);

  // Central rotating cube (N64 blue)
  console.log('[babylon-demo] Created cube:', cube);
  cube = nova64.scene.createCube(2, 0x0055ff, [0, 1, 0]);

  // Orbiting sphere (warm orange)
  sphere = nova64.scene.createSphere(0.7, 0xff8800, [4, 1, 0], 12);

  // Tall pillar
  pillar = nova64.scene.createCylinder(0.3, 0.3, 3, 0x888888, [-3, 1.5, -3]);

  // Decorative cone on top of pillar
  cone = nova64.scene.createCone(0.5, 1, 0xff2266, [-3, 3.5, -3]);

  // A dynamic point light attached to the orbiting sphere
  orbitLight = nova64.light.createPointLight(0xff8800, 1.5, 4, 1, 0);
}
...
export function update(dt) {
  ...
  nova64.scene.rotateMesh(cube, dt * 0.5, dt * 0.8, dt * 0.3);
  ...
  nova64.scene.setPosition(sphere, sx, sy, sz);

  // Move point light with the sphere
  nova64.scene.setPosition(orbitLight, sx, sy, sz);

  // Slowly spin the cone
  nova64.scene.rotateMesh(cone, 0, dt * 0.6, 0);

  // Gentle camera bob
  nova64.camera.setCameraPosition(0, 6 + Math.sin(time * 0.3) * 0.4, 12);
}

export function draw() {
  // HUD overlay (2D, drawn via Canvas2D)
  nova64.draw.print('NOVA64 BABYLON DEMO', 12, 12, 0x88aaff, 14);
  nova64.draw.print(`FPS: ${fps}`, 12, 36, 0x44dd88, 12);
  nova64.draw.print(`Score: ${score}`, 12, 54, 0xffdd44, 12);
  nova64.draw.print(`Time: ${time.toFixed(1)}s`, 12, 72, 0xaaaaaa, 12);

  // Stats
  const stats = nova64.scene.get3DStats();
  nova64.draw.print(`Meshes: ${stats.meshes}`, 12, 96, 0x666688, 11);
  nova64.draw.print(`Backend: ${stats.backend}`, 12, 112, 0x666688, 11);

  // Credit
  nova64.draw.print('Babylon.js Phase 2', 12, 460, 0x444466, 11);
}
